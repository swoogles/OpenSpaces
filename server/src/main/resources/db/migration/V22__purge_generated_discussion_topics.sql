-- V22__purge_generated_discussion_topics.sql
-- Hard-delete junk/test discussions created from DiscussionTopics.entries.
-- Purges materialized rows plus related action/event logs that are not fully FK-linked.

WITH topic_entries(topic) AS (
    VALUES
...
matching_topic_ids AS (
    SELECT id
    FROM discussions
    WHERE topic IN (SELECT topic FROM topic_entries)
),
deleted_confirmed_actions AS (
    DELETE FROM confirmed_actions
    WHERE entity_type = 'Discussion'
      AND (
          (payload ->> 'topicId')::bigint IN (SELECT id FROM matching_topic_ids)
          OR (payload -> 'discussion' ->> 'id')::bigint IN (SELECT id FROM matching_topic_ids)
          OR (payload ->> 'topic1')::bigint IN (SELECT id FROM matching_topic_ids)
          OR (payload ->> 'topic2')::bigint IN (SELECT id FROM matching_topic_ids)
      )
    RETURNING id
),
deleted_discussion_events AS (
    DELETE FROM discussion_events
    WHERE topic_id IN (SELECT id FROM matching_topic_ids)
    RETURNING id
)
DELETE FROM discussions
WHERE id IN (SELECT id FROM matching_topic_ids);
